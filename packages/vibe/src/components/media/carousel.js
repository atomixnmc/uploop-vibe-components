// ─── Vibe Carousel — Cross-fade Image Carousel ──────────────
//
...
//
// A skeleton shimmer underneath fills load-time gaps.
...
export const ImageCarousel = component('VibeCarousel', {
  state: {
    index: 0,
    prevIndex: 0,
    auto: true,
  ...
  update: {
    ...
    select: (s, i) => ({
      ...s,
      prevIndex: s.index,
      index: typeof i === 'number' ? i : parseInt(i, 10),
      auto: false,
    }),
  ...
  view(state) {
    ...
    return `<div style="font-family:sans-serif;padding:1rem;text-align:center;">
      <!-- Fixed container -->
      <div style="position:relative;width:100%;max-width:600px;margin:0 auto;
        ...
        <div style="position:absolute;inset:0;z-index:0;
                    ...
                    animation:up-shimmer 1.8s ease-in-out infinite;"></div>

        <!-- From layer: previous slide fading OUT -->
        ${isTransition ? `<div style="position:absolute;inset:0;z-index:1;
                    background-image:url(${previous.src});
                    background-size:cover;background-position:center;
                    animation:carousel-fade-out 0.5s ease forwards;"
                    role="img" aria-label="${previous.alt}"></div>` : ''}
        ...
        <!-- Nav buttons -->
        <button @click="prev" style="position:absolute;left:8px;top:50%;z-index:10;
                    transform:translateY(-50%);
                    background:rgba(0,0,0,0.35);color:white;border:none;
                    border-radius:50%;width:36px;height:36px;
                    cursor:pointer;font-size:1.2rem;backdrop-filter:blur(4px);">&#8249;</button>
        <button @click="next" style="position:absolute;right:8px;top:50%;z-index:10;
                    transform:translateY(-50%);
                    background:rgba(0,0,0,0.35);color:white;border:none;
                    border-radius:50%;width:36px;height:36px;
                    cursor:pointer;font-size:1.2rem;backdrop-filter:blur(4px);">&#8250;</button>
      ...
      <div style="margin-top:0.75rem;display:flex;gap:0.5rem;justify-content:center;align-items:center;">
        ${IMAGES.map((_, i) => `<button @click="select:${i}" style="
          width:10px;height:10px;border-radius:50%;border:none;cursor:pointer;
          background:${i === state.index ? '#646cff' : '#ddd'};
          transition:background 0.3s;"></button>`).join('')}
        <button @click="toggleAuto" style="
          margin-left:0.5rem;padding:0.2rem 0.6rem;border:1px solid #ccc;
  ...
  mount: (el, ctx) => {
    ...
    _intervalId = setInterval(() => ImageCarousel.loop.send('tick'), 3000)
